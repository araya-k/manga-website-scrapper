    ...
    paginatedResult: (model) => {
        return async (req, res, next) => {
            ...
            if (isNaN(page) && isNaN(limit)) {
                try {
                    const allResult = await model.find({ 'seriesSlug': req.params.slug }, { '_id': 0, '__v': 0 }).sort({ 'chapterSlug': 1 })
                ...
                try {
                    paginatedResult.results = await model.find({ 'seriesSlug': req.params.slug }, { '_id': 0, '__v': 0 }).sort({ 'chapterSlug': 1 }).limit(limit).skip(startIndex)
